...
/**
*	split Left and Right channel of pcm16le file.
*
*
*
**********/
...
{
	FILE *fp = fopen(url, "rb+");
	FILE *fpl = fopen("output_left_channel.pcm", "wb");
	FILE *fpr = fopen("output_right_channel.pcm", "wb");

	unsigned char* sample = (unsigned char*)malloc(4);
	while (!feof(fp))
	{
		fread(sample, 4, 1, fp);
		fwrite(sample, 2, 1, fpl);
		fwrite(sample + 2, 2, 1, fpr);
	}
...
/**
*	halve Left channel volume of pcm16le file.
*
*
*
**********/
...
{
	FILE *fp = fopen(url, "rb+");
	FILE *fpOut = fopen("output_havf_volume.pcm", "wb");

	unsigned char* sample = (unsigned char*)malloc(4);
	while (!feof(fp))
	{
		fread(sample, 4, 1, fp);
		short* l = (short*)sample;
		*l = *l / 2;
		fwrite(sample, 2, 1, fpOut);
		fwrite(sample + 2, 2, 1, fpOut);
	}
...
{
	FILE *fp = fopen(url, "rb+");
	FILE *fpOut = fopen("output_double_speed.pcm", "wb");
...
/**
*	convert pcm16 data to pcm8 data
*
*
*
**********/
...
{
	FILE *fp = fopen(url, "rb+");
	FILE *fpOut = fopen("output_pcm8.pcm", "wb");

	unsigned char* sample = (unsigned char*)malloc(4);
	while (!feof(fp))
	{
		fread(sample, 4, 1, fp);
		short* left = (short*)sample;
		short* right = (short*)(sample + 2);
		unsigned char sample8l = *left / 256 + 128;
		unsigned char sample8r = *right / 256 + 128;
		fwrite(&sample8l, 1, 1, fpOut);
		fwrite(&sample8r, 1, 1, fpOut);
	}
	free(sample);
	fclose(fp);
	fclose(fpOut);
...
{
	FILE *fp = fopen(url, "rb+");
	FILE *fpOut = fopen("output_cut.pcm", "wb");
	...
	while (!feof(fp))
	{
		cnt++;
		fread(sample, 2, 1, fp);
		if (cnt > startpos && cnt <= (startpos + num))
		{
			fwrite(sample, 2, 1, fpOut);
